...
const assignato_lib = require('./misc_lib.js');
...
async function createBasicReport(userid, key) {
    ...
    var code = await assignato_lib.getUserShareCode(userid, null);
    if (code == undefined)
        return;
    ...
    saveReport(code, key, assignments);
}
...
async function saveReport(code, key, assignments) {
    var cnx = await mongo.connect(config.mongo_url);
    var collec = await cnx.db(code).collection(key);
    var res = await collec.insertMany(assignments);
    console.log("Wrote " + res.result.n + " of " + assignments.length + " documents for share code " + code + " with report ID " + key);
    collec = await cnx.db(code).collection("reports");
