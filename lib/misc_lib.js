...
function getAmbiguousCollectionAndFilter(body, res) {
    var idx = ["crn", "title", "name", "building"].filter(x => Object.keys(body).includes(x));
    if (idx.length != 1
        || (idx[0] == "crn" && !validationFuncs.numeric(body, ["crn"]))
        || (idx[0] == "title" && !validationFuncs.strings(body, ["title"]))
        || (idx[0] == "name" && !validationFuncs.strings(body, ["name"]))
        || (idx[0] == "building" && (!validationFuncs.strings(body, ["building"]) || !validationFuncs.strings(body, ["number"])))) {
        res.send(defaults.validation_error);
        return { collec_name: undefined, filter: undefined };
    }
    var collec_name = {
        crn: "classes",
        title: "items",
    ...
    var filter = {};
    filter[idx[0]] = body[idx[0]];
    if (idx[0] == "building")
        filter.number = body.number;
    return {
        collec_name: collec_name,
        filter: filter
    }
}
...
module.exports = {
    ...
    getAmbiguousCollectionAndFilter: getAmbiguousCollectionAndFilter
}